...
console.log('\n-- 7. gece biterken odada oturanlar --')
{
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (/Yıllığa bak/.test(after)) break
  }
  if (!/Yıllığa bak/.test(after)) {
  ...
  if (/Birazdan başlıyoruz/.test(after)) bad('bitiş', 'the room is told it is about to start, after it ended')
